    ...
    <>
      <Tabs >
        <Menu>
          ...
          >
            {tabsList &&
              tabsList?.slice(0, 5).map((category, index) => (
                <Tab
                  p="17px 8px 8px"
                  mb="0"
                  _selected={{
                    mb: "0",
                    borderBottom: "2px solid #6863F3",
                    color: "#6863F3",
                  }}
                  onClick={() => filterUsingTabs(category?.id)}
                  key={index}
                >
                  {category?.name}
                </Tab>
              ))}
            {tabsList && tabsList?.length >= 5 ? (
              <Tab
                _active={{ bg: "transparent", color: "#6863F3" }}
                _hover={{ bg: "transparent", color: "#6863F3" }}
                whiteSpace="nowrap"
                as={MenuButton}
                p={{ base: "15px 8px 8px", md: "17px 8px 8px" }}
              >
                More{" "}
          ...
          >
            {tabsList &&
              tabsList?.slice(5, tabsList.length).map((category, index) => {
                return (
                  <MenuItem
          ...
          {data?.length <= 0 ? (
            ...
            data?.map((collection: any, index: number) => {
              return (
                <Box
                  cursor="pointer"
                  width={{ base: "100%", sm: "50%", md: "33%", xl: "25%" }}
                  key={index}
                  onClick={() => router.push(`/collection/${collection?.id}`)}
                >
                  <CollectionCard
                    type="withBody"
                    featureImage={collection?.logoImageUrl}
