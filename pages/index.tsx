    ...
    <div>
      ...
      <Heading px={{ base: '0', md: '0 17px',xl:'0' }} fontSize={{base:'24px',md:'36px',xl:'48px'}}>Featured Collections</Heading>
       ...
       { data?.featured && data?.featured.map((item:collectionType, index:number)=>{
                return <Link href={`collection/${item?.id}`} key={index}><CollectionCard type='withoutBody' featureImage={item?.logoImageUrl}  isShowFeatureImage={true} isShowLogoImage={false} name={item?.name} /></Link>
            })}
        </SlickSlider>
      </Container>
      <Container maxW={{ sm: 'xl', md: '3xl', lg: '5xl', xl: '8xl' }} mt={{ base: '40px', lg: '80px' }} >
        <Flex justifyContent='space-between' alignItems='center' mb='40px'>
          ...
          <Button p={{ base: '15px', md: '20px 32px' }}  variant='primary'><Link href='/categories'>View All</Link></Button>
        </Flex>
        <FilterTabs
          tabsList={categories}
      ...
      <Container maxW={{ sm: 'xl', md: '3xl', lg: '5xl', xl: '8xl' }} mt={{ base: '40px', lg: '80px' }}>
        ...
        <SlickSlider>
            {data?.trending?.map((item:collectionType, index:number)=>{
                return <Link href={`collection/${item?.id}`} key={index}><CollectionCard type='withBody' featureImage={item?.logoImageUrl}  isShowFeatureImage={true} isShowLogoImage={false} name={item.name} /></Link>
            })}         
        </SlickSlider>
      </Container>
 <Container maxW={{ sm: 'xl', md: '3xl', lg: '5xl', xl: '8xl' }} mt={{ base: '40px', lg: '80px' }}>
